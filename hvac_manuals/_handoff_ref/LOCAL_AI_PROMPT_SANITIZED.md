...
A later targeted 6-row JCI packaged-rooftop probe also downloaded and verified all 6 samples with `High confidence` without JCI/JACI login. The successful routes were:

- Johnson Controls public Fluid Topics API for docs pages.
- HVACNavigator public PDF URLs.

For Johnson Controls docs, the useful anonymous API pattern is:

1. Search: `POST https://docs.johnsoncontrols.com/ductedsystems/api/khub/clustered-search` with JSON like `{"query":"AD15"}`.
2. Read document metadata: `GET https://docs.johnsoncontrols.com/ductedsystems/api/khub/documents/{documentId}`.
3. Download PDF content: `GET https://docs.johnsoncontrols.com/ductedsystems/api/khub/documents/{documentId}/content`.

The session can be unauthenticated and still return public documents. A docs page may look like a JavaScript viewer in raw HTML; that is not the same as a login wall.

Practical strategy for JCI:

1. Search the public Fluid Topics API and use `{documentId}/content` for PDF download.
2. Search for direct PDFs on `files.hvacnavigator.com/p/*.pdf`.
3. Search public docs pages under `docs.johnsoncontrols.com/ductedsystems/`.
4. For docs pages that load as JavaScript viewers, use the API route above instead of treating the page as blocked.
5. Use third-party datasheet pages only as a bridge to the real PDF URL.
6. If a product portal requires login, do not stop immediately; try direct PDF titles, manual numbers, and model ranges.
7. If only a login-only page is found and no direct PDF can be verified, mark `Needs human confirmation`.

Known verified JCI examples:

- `AD15/AD18/AD28`: Johnson Controls Choice AD15 to AD28 Technical Guide via public docs API.
- `JH35/JH40/JV30`: Select JV28 to JV50 / JH28 to JH50 Technical Guide via public HVACNavigator PDF.

For `files.hvacnavigator.com`, Python SSL verification can fail. If that happens, retry the request with `verify=False`, then validate that the file starts with `%PDF` and that extracted text proves the match.
