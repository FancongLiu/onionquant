...
wb = openpyxl.load_workbook(
    "E:/HVAC_PDF_search/Unitary_pdf_manual_search_0519_lite.xlsx"
)
ws = wb.active
...
team_data = defaultdict(
    lambda: {"total": 0, "found": 0, "high_conf": 0, "not_found_rows": []}
)
...
lines.append("=" * 80)
lines.append(
    "HVAC Product Manual PDF Search - Remaining Not Found Items - HANDOFF PROMPT"
)
lines.append("=" * 80)
lines.append("")
...
lines.append("")
lines.append(
    "From 482 HVAC product manual search requirements, 385 rows (80%) have been found."
)
lines.append(
    "97 rows remain not found, corresponding to 42 unique model series across 6 brands."
)
lines.append(
    "All public channels (manufacturer sitemaps, CDNs, APIs, document portals) have been exhausted."
)
lines.append("")
lines.append("The completed results are in: Unitary_pdf_manual_search_RESULTS.xlsx")
lines.append(
    "The tracking manifest is in: hvac_manuals/manifest.json (138/180 series found)"
)
lines.append("")
...
for brand in ["RHEEM", "RUUD", "CARRIER", "BRYANT", "BOSCH", "JOHNSON CONTROLS"]:
    brand_series = {
        k: v for k, v in all_not_found_series.items() if v.get("brand") == brand
    }
...
for key, entry in sorted(all_not_found_series.items()):
    ...
    if brand == "JOHNSON CONTROLS":
        if series_name in ["KC", "KD", "KE", "WC", "WD", "WE"]:
            lines.append(
                "- **Breakthrough Suggestion**: Commercial split systems. Try searching exact model digits + 'technical guide' + 'pdf'. JCI Fluid Topics API already tried (docs.johnsoncontrols.com/ductedsystems/api/khub/clustered-search) - no results. These are identical to Bosch KC/KD/KE/WC/WD/WE - also not found."
            )
        else:
            lines.append(
                f"- **Breakthrough Suggestion**: Try '{series_name}' + 'packaged rooftop' + 'pdf' + 'site:johnsoncontrols.com'"
            )
    elif brand in ["RHEEM", "RUUD"]:
        lines.append(
            f"- **Breakthrough Suggestion**: {brand} legacy/discontinued model. Try sitemap (rheem.com/wp-sitemap-products-1.xml) or direct model number search. May be OEM identical to another brand's model."
        )
    elif brand == "CARRIER":
        lines.append(
            "- **Breakthrough Suggestion**: Try shareddocs.com or carrier.com product page. Legacy commercial equipment may have archived PDFs on other sites like manualslib.com or archive.org."
        )
    elif brand == "BRYANT":
        lines.append(
            f"- **Breakthrough Suggestion**: Bryant = Carrier OEM. Search for identical Carrier model specs. 'bryant {series_name} technical guide pdf'"
        )
    elif brand == "BOSCH":
        lines.append(
            "- **Breakthrough Suggestion**: Try bosch-thermotechnology.com product search or full model name + 'pdf'. May have been discontinued/replaced by newer R-454B generation."
        )
...
for team in sorted(team_data.keys()):
    ...
    if d["not_found_rows"]:
        ...
        for item in d["not_found_rows"]:
            lines.append(
                f"  - RefId={item['ref_id']} | {item['brand']} | {item['model'][:45]} | {item['capacity']} | {item['refrigerant']} | Status={item['status']}"
            )
        lines.append("")
...
lines.append("")
lines.append(
    "1. **Structured sources first**: Always check manufacturer sitemaps/APIs/CDNs before web search."
)
lines.append("2. **JCI Fluid Topics API** (public, no auth):")
lines.append(
    '   - Search: `POST https://docs.johnsoncontrols.com/ductedsystems/api/khub/clustered-search` with JSON `{"search": "<model>"}`'
)
lines.append(
    "   - Download: `GET https://docs.johnsoncontrols.com/ductedsystems/api/khub/documents/{documentId}/content`"
)
lines.append(
    "3. **Rheem/Ruud**: Sitemap at `rheem.com/wp-sitemap-products-1.xml` -> product page -> extract embedded DocumentURL"
)
lines.append(
    "4. **Carrier/Bryant**: `shareddocs.com` + `carrier.com` product pages -> find embedded PDF URLs"
)
lines.append("5. **Bosch**: `bosch-thermotechnology.com` product search")
lines.append(
    "6. **Legacy/Discontinued Models**: If not in current sitemap, try `archive.org` / `web.archive.org` for historical product page snapshots."
)
...
lines.append(
    "8. **If login is truly required**: Mark as `Needs human confirmation` and specify exactly what account/permission is needed."
)
lines.append(
    "9. **Verification is mandatory**: Download PDF, extract text, verify model + refrigerant + capacity match. Do not trust search result titles alone."
)
lines.append("")
lines.append("Generated: 2026-05-26")
lines.append(
    "Data Source: manifest.json (138/180 series found), Excel (385/482 rows found)"
)
lines.append("PDFs downloaded: hvac_manuals/pdf_downloads/ (~94 PDFs)")
lines.append(
    "Fluid Topics API doc list: hvac_manuals/jci_fluid_topics_docs.json (1208 docs)"
)
...
for team in sorted(team_data.keys()):
    ...
    pct = d["found"] / d["total"] * 100 if d["total"] > 0 else 0
    print(
        f"  {team}: {d['found']}/{d['total']} ({pct:.0f}%) found, {len(d['not_found_rows'])} not_found, {d['high_conf']} high-confidence"
    )
