    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        for page in pdf.pages:
            ...
            for i, line in enumerate(lines):
                ...
                if any(kw in context.lower() for kw in ['capacity', 'eer', 'ieer', 'gross', 'net capacity', 'btu']):
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                if any(kw in header_text.lower() for kw in
                       ['capacity', 'eer', 'ieer', 'tonnage', 'nominal', 'model', 'btu', 'gross']):
                    tables_data.append({
                        'page': page.page_number,
                        'table': table,
...
def match_specs(pdf_specs: dict, excel_rows: list[dict]) -> list[dict]:
    ...
    for row in excel_rows:
        ...
        excel_capacity = str(row['CoolingCapacity95FSearchULE']).split('/')[0] if row['CoolingCapacity95FSearchULE'] else ''
        excel_eer = str(row['EER95FSearchULE']).split('/')[0] if row['EER95FSearchULE'] else ''
        excel_ieer = str(row['IEERSearchUle']).split('/')[0] if row['IEERSearchUle'] else ''
        excel_refrigerant = str(row['RefrigerantTypeUle']).strip() if row['RefrigerantTypeUle'] else ''
        ...
        results.append({
            ...
            'confidence': 'HIGH' if spec_match_score >= 2 else ('MEDIUM' if spec_match_score >= 1 else 'LOW'),
        })
...
def validate_against_excel(pdf_path: Path, excel_rows: list[dict]) -> dict:
    ...
    for m in matched[:10]:
        ...
        print(f"  {flag} {m['model']}: cap={m['excel_capacity']}, eer={m['excel_eer']}, "
...
if __name__ == '__main__':
    ...
    rows = [dict(zip(headers, row)) for row in ws.iter_rows(min_row=2, max_row=ws.max_row, values_only=True)]
