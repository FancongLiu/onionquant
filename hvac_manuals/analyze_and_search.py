...
def normalize_query(query):
    ...
    if not query:
        return (None, None)
    parts = query.split(' packaged rooftop')[0].strip() if 'packaged rooftop' in query else query
    # Extract brand and model
    for brand in ['JOHNSON CONTROLS', 'BOSCH', 'CARRIER CORPORATION',
                   'BRYANT HEATING & COOLING SYSTEMS', 'LENNOX', 'RUUD', 'RHEEM']:
        if brand in parts:
...
def group_by_manual(rows):
    ...
    for r in rows:
        if r['pdf_link'] and r['pdf_link'] not in ('untitled', '/'):
            ...
            pdf_link_groups[f"_UNKNOWN_{r['Brand_Scope_Label']}_{r['Suggested_Manual_Query'][:40]}"].append(r)
    ...
    for r in rows:
        ...
        if brand and series:
            ...
            brand_series_groups[('_UNGROUPED', str(r['Suggested_Manual_Query'])[:60])].append(r)
...
def main():
    ...
    print()
    # Show top groups
    for (brand, series), group_rows in sorted(brand_series_groups.items(),
