...
def load_m(): return json.loads(open(MANIFEST_PATH, "r", encoding="utf-8").read())
def save_m(m): json.dump(m, open(MANIFEST_PATH, "w", encoding="utf-8"), indent=2, ensure_ascii=False)
...
def download_pdf(url, outpath):
    if outpath.exists() and outpath.stat().st_size > 1000: return True
    try:
...
def verify_pdf(pdf_path, entry):
    ...
    except ImportError: return {"verdict": "Not verified", "score": 0, "evidence": "pdfplumber missing"}
    try:
    ...
    if len(text) < 100: return {"verdict": "Not found", "score": 0, "evidence": "Text too short"}
    ...
    for a in aliases.get(brand, [brand]):
        if a in tu: evidence.append(f"Brand:{a}"); score += 5; break
    if score == 0: evidence.append(f"Brand NOT found:{brand}")

    mm = False
    for mdl in models:
        if mdl and mdl.upper() in tu: evidence.append(f"Model:{mdl}"); score += 5; mm = True; break
    if not mm:
        if series and series.upper() in tu: evidence.append(f"Series:{series}"); score += 3
        else: evidence.append(f"Model NOT found:{models[:3]}")

    rm = False
    for r in refs:
        if not r: continue
        for v in [r, r.replace("-",""), r.replace("-"," ")]:
            if v.upper() in tu: evidence.append(f"Refrigerant:{r}"); score += 5; rm = True; break
        if rm: break
    ...
    for c in caps:
        ...
        for n in re.findall(r"(\d+)", str(c))[:2]:
            if n in text: evidence.append(f"Capacity:{n}BTU"); score += 5; cm = True; break
        if cm: break

    if re.search(r'EER\s*[:\s]*\d+\.?\d*', tu) or re.search(r'IEER\s*[:\s]*\d+\.?\d*', tu):
        evidence.append("Performance data present"); score += 5
...
def process_brand(entries, brand_name, strategy="sitemap"):
    ...
    for i, (mid, entry) in enumerate(entries):
        if entry.get("status") == "found": continue
        series = entry["series"]
        models = entry.get("models", [])
                ...
                for m in models[:2]:
                    for url in [
                        ...
                        try:
                            ...
                            if r.status_code == 200: candidates.append(url)
                        except: pass

            elif brand_name == "BOSCH":
                cdn = "https://www.bosch-homecomfort.com/us/media/country_pool/documents"
                for m in models[:2]:
                    tk = model_token(m)
                    for path in [
                        ...
                        try:
                            ...
                            if r.status_code == 200: candidates.append(path)
                        except: pass
                        ...
                        if r.status_code == 200:
                            ...
                            if isinstance(data, list):
                                for doc in data[:3]:
                                    did = doc.get("id","")
                                    if did:
                                        candidates.append(f"https://docs.johnsoncontrols.com/ductedsystems/api/khub/documents/{did}/content")
                    except: pass
        ...
        if not success:
            ...
            entry["evidence"] = f"No structured source match ({len(candidates)} candidates)"
...
def main():
    ...
    for b in ["RHEEM","RUUD","BOSCH","CARRIER","BRYANT","JOHNSON CONTROLS"]:
        ...
        fs = sum(1 for v in m.values() if v.get("brand")==b and v.get("status")=="found")
        if bs > 0: log.info(f"  {b}: {fs}/{bs}")
