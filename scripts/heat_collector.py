...
def collect_market_heat() -> dict | None:
    ...
    try:
        ...
        return {
            ...
            "finviz_unusual": [
                {"ticker": r["ticker"], "change_pct": r.get("change_pct", "?")}
                for r in finviz[:15]
            ],
        }
    except Exception as e:
...
def collect_round() -> dict:
    ...
    if heat:
        ...
        for s in ai[:5]:
            ...
            print(
                f"  {s['ticker']:<6} mentions:{s['mentions']:>4} "
    ...
    if market:
        ...
        if unusual:
            ...
            for v in unusual[:5]:
                print(
                    f"  {v['ticker']:<6} {v['volume_ratio']:.1f}x "
...
def main_loop(interval_minutes: int = INTERVAL_MINUTES):
    """持续采集主循环."""
    print("  OnionQuant Heat Collector")
