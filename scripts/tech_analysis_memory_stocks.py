...
def generate_report(results: list) -> str:
    ...
    lines = [
        f"# Memory/Semiconductor Technical Analysis Report",
        f"**Generated**: {now}",
        f"**Context**: Wall Street banks restricted leverage on Samsung/SK Hynix/TSMC on June 12, 2026",
        "",
        "---",
        "",
    ]
    ...
    for r in results:
        ...
        if r["ma200"] is not None:
            ...
            lines.append(f"| **MA-200** | N/A | < 200 days data |")
        ...
        # Support / Resistance
        if r["support_levels"]:
            lines.append(f"**Support Levels**: " + " | ".join(f"{s:,.2f}" for s in r["support_levels"]))
        else:
            lines.append(f"**Support Levels**: None detected below current price")
        if r["resistance_levels"]:
            lines.append(f"**Resistance Levels**: " + " | ".join(f"{s:,.2f}" for s in r["resistance_levels"]))
        else:
            lines.append(f"**Resistance Levels**: None detected above current price")

        lines.append("")

        lines.append(f"| Period | Return |")
        lines.append(f"|--------|--------|")
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*Report auto-generated by scripts/tech_analysis_memory_stocks.py*")
