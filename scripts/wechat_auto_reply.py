...
def _get_token():
    r = requests.get(f"{WECOM_API}/gettoken?corpid={CORP_ID}&corpsecret={SECRET}", timeout=10)
...
def _send_wechat(text):
    ...
    r = requests.post(f"{WECOM_API}/message/send?access_token={token}", json=body, timeout=10)
    result = r.json()
