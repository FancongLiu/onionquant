...
ALERT_FILE = OUTBOX_DIR / f"ALERT_connectivity_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def check_tmux():
    try:
        r = subprocess.run(["tmux", "has-session", "-t", "onionquant"],
                           capture_output=True, timeout=10)
        if r.returncode != 0:
            return False, "onionquant 会话不存在"

        # Check if claude process is running inside tmux
        r2 = subprocess.run(["bash", "-c", "ps aux | grep -c '[c]laude'"],
                            capture_output=True, timeout=10)
...
def check_dashboard():
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            return True, f"depts={data.get('departments', '?')} inbox={data.get('inbox_pending', '?')}"
...
def check_message_flow():
    ...
    # Check inbox pending
    pending = [f for f in INBOX_DIR.glob("MSG_*.md")
               if f.name != "README.md" and not (PROCESSED_DIR / f"{f.name}.processing").exists()]

    status_parts = []
    if stale_locks:
        status_parts.append(f"{len(stale_locks)} 孤儿锁")
        ALERTS.append(("孤儿锁文件", f"发现 {len(stale_locks)} 个超过30分钟的.processing锁: {', '.join(stale_locks[:5])}"))
...
def check_deepseek_api():
    ...
    try:
        r = subprocess.run(["bash", "-c", "ps aux | grep -c '[c]laude'"],
                           capture_output=True, timeout=10)
        claude_count = int(r.stdout.decode().strip() or "0")
        if claude_count > 0:
            return True, f"Claude CLI 运行中 (API 可用)"
    except Exception:
        pass
...
def check_tunnels():
    try:
        r = subprocess.run(["bash", "-c", "pgrep -a cloudflared 2>/dev/null"],
                           capture_output=True, timeout=10)
...
def restart_hermes():
    ...
    try:
        ...
        subprocess.run(["rm", "-f",
                       ...
                       timeout=5)
        subprocess.Popen(["nohup", "hermes", "gateway", "run", "--replace"],
    ...
    for name, (fn, severity) in CHECKS.items():
        try:
            ...
            if ok is None:
                ...
                results[name] = {"status": "fail", "severity": severity, "detail": detail}
        except Exception as e:
