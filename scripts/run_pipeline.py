...
def step1_fetch(tickers: list, start: str, end: str = None, save_parquet: bool = True):
    ...
    if save_parquet:
        ...
        data_dir = (
            Path(__file__).resolve().parent.parent / "quant_framework" / "data" / "raw"
        )
...
def step3_signals(factor_df, price_series=None):
        ...
        for dt, grp in signals.groupby("date"):
            ...
            if len(over_sectors) > 0:
                ...
                for sec, total_wt in over_sectors.items():
                    sec_mask = (signals["date"] == dt) & (
                        signals["ticker"].isin(grp.loc[grp["sector"] == sec, "ticker"])
                    )
    ...
    per_date_counts = (
        signals.groupby("date").size() if "date" in signals.columns else pd.Series()
    )
    if not per_date_counts.empty and (per_date_counts < MIN_POSITIONS).any():
        ...
        for dt in per_date_counts[per_date_counts < MIN_POSITIONS].index:
            extra = pd.DataFrame(
                {
                    "date": dt,
                    "ticker": list(all_tickers[:MIN_POSITIONS]),
                    "signal": 1,
                    "weight": 1.0 / MIN_POSITIONS,
                    "combined_score": 0.0,
                }
            )
    ...
    print(
        f"       {n_long} long signals ({avg_positions:.1f}/day avg, "
...
def main():
    ...
    default_tickers = (
        ",".join(dynamic_tickers) if dynamic_tickers else "AAPL,MSFT,NVDA,GOOGL,AMZN"
    )
    ...
    source = (
        "TASK_TRACKER.md"
        if dynamic_tickers and args.tickers == default_tickers
        else "CLI"
    )
    print(f"\n{'=' * 56}")
    print(
        f"  Quant Pipeline — {len(tickers)} tickers (source: {source}), {args.start} → {args.end or 'today'}"
    )
    print(f"{'=' * 56}\n")
    ...
    try:
        ...
        try:
            signals = step3_signals(
                factors, df["close"] if "close" in df.columns else None
            )
        except Exception as e:
        ...
        if args.mode == "tune":
            ...
            try:
                ...
                def strategy_sim(**params):
                    ...
                    sim_signals = step3_signals(
                        sim_factors, df["close"] if "close" in df.columns else None
                    )
                    sim_bt = step4_backtest(sim_signals, df)
                    return {
                        "sharpe": sim_bt.get("sharpe", 0),
                        "return": sim_bt.get("total_return", 0),
                    }
                ...
                tune_path = (
                    ...
                    / "company"
                    / "reports"
                    / f"tune_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
                )
