...
def parse_task_tracker(path: Path) -> List[dict]:
    ...
    for section in sections:
        ...
        for match in row_pattern.finditer(section):
            ...
            if "✅" in status_raw and not any(e in status_raw for e in active_emojis - {"✅"}):
                continue
    ...
    for task in tasks:
        ...
        if task_key in existing:
            ...
            if current_status != task["status"] and task["status"] in (
                ...
                print(f"  UPDATE {kanban_id}: {current_status} → {task['status']} ({task_key})")
                updated += 1
            else:
                skipped += 1
        else:
            ...
            print(f"  CREATE {task_key} → kanban [{task['status']}] (P{task['priority']})")
            created += 1
...
def main():
    ...
    print(f"🔍 Fetching existing kanban tasks...")
