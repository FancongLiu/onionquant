...
from quant_framework.backtest.harness import vectorized_backtest, signal_backtest
...
CATALYST_CALENDAR = {
    "DXYZ": [
        {"date": "2026-05-19", "event": "Starship IFT-12 发射", "impact": "binary", "magnitude": 0.35},
        {"date": "2026-06-12", "event": "SpaceX IPO S-1 公开", "impact": "positive", "magnitude": 0.25},
    ],
    "NVDA": [
        {"date": "2026-05-20", "event": "Q1 FY2027 财报", "impact": "binary", "magnitude": 0.08},
    ],
    "MU": [
        {"date": "2026-05-21", "event": "三星罢工开始", "impact": "positive", "magnitude": 0.05},
    ],
    "RKLB": [
        {"date": "2026-06-01", "event": "Neutron 首飞", "impact": "binary", "magnitude": 0.30},
    ],
    "AVGO": [
        {"date": "2026-06-11", "event": "Q2 FY2026 财报", "impact": "binary", "magnitude": 0.06},
    ],
    "AMD": [
        {"date": "2026-07-28", "event": "Q2 FY2026 财报", "impact": "binary", "magnitude": 0.07},
    ],
    "MRVL": [
        {"date": "2026-05-29", "event": "Q1 FY2027 财报", "impact": "binary", "magnitude": 0.06},
    ],
    "LITE": [
        {"date": "2026-08-12", "event": "Q4 FY2026 财报", "impact": "binary", "magnitude": 0.06},
    ],
    "COHR": [
        {"date": "2026-05-15", "event": "Q3 FY2026 财报 (小幅beat引发抛售)", "impact": "binary", "magnitude": 0.05},
    ],
    "ANET": [
        {"date": "2026-07-28", "event": "Q2 FY2026 财报", "impact": "binary", "magnitude": 0.06},
    ],
}
...
def compute_factor_scores(close: pd.DataFrame, returns: pd.DataFrame,
                          catalysts: dict = None) -> pd.DataFrame:
    ...
    for ticker in close.columns:
        ...
        # 1. Momentum — 简单累计收益
        mom_5d = float((price.iloc[-1] / price.iloc[-5] - 1) * 100) if len(price) >= 5 else 0
        mom_20d = float((price.iloc[-1] / price.iloc[-20] - 1) * 100) if len(price) >= 20 else 0
        ...
        cat_score = sum(c["magnitude"] * (1 if c["impact"] == "positive" else 0.5)
...
def run_regime_detection(returns: pd.DataFrame) -> dict:
    ...
    try:
        ...
        regime = {
            ...
            "current_regime": rolling["regime"].iloc[-1] if len(rolling) > 0 else "unknown",
...
def run_risk_threshold_engine(factor_df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "actions": [
            {"type": a.action_type, "ticker": a.ticker, "magnitude": a.magnitude, "rationale": a.rationale}
...
def run_binary_catalyst_backtest(ticker: str, event_dates: list,
                                  close: pd.Series, window_days: int = 10) -> dict:
    ...
    return {
        ...
        "sharpe_events": round(avg_return / ret_series.std(), 2) if ret_series.std() > 0 else 0,
    }
...
def generate_decision_table(factor_df: pd.DataFrame, rte_result: dict) -> list:
    ...
    for _, row in factor_df.iterrows():
        ...
        # Score components
        mom_score = 3 if row["mom_5d_pct"] > 3 else (-2 if row["mom_5d_pct"] < -5 else 0)
        vol_penalty = -1 if row["ann_vol"] > 0.8 else (1 if row["ann_vol"] < 0.4 else 0)
        sharpe_bonus = 2 if row["sharpe_6m"] > 1.0 else (-1 if row["sharpe_6m"] < -0.5 else 0)
        catalyst_bonus = row["catalyst_count"] * 1.5 if row["catalyst_score"] > 0 else -1

        # Regime adjustment from risk-threshold-engine
        regime_mult = {
            "LOW": 1.2, "MODERATE": 1.0, "ELEVATED": 0.7, "SEVERE": 0.4
        }.get(rte_result["regime"], 1.0)

        composite = (mom_score * 0.2 + vol_penalty * 0.15 + sharpe_bonus * 0.25
                    + catalyst_bonus * 0.3 + (-1 if row["beta_spx"] > 1.5 else 0) * 0.1) * regime_mult
...
def build_supply_chain_report(ticker: str) -> dict:
    ...
    SUPPLY_EDGES = {
        "NVDA": {"suppliers": ["MU", "SNDK", "LITE", "COHR"], "customers": [], "competitors": ["AMD", "AVGO"]},
        "MU": {"suppliers": [], "customers": ["NVDA", "AVGO"], "competitors": ["SNDK", "AMD"]},
        "COHR": {"suppliers": [], "customers": ["NVDA"], "competitors": ["LITE"]},
        "LITE": {"suppliers": [], "customers": ["NVDA", "AVGO"], "competitors": ["COHR"]},
        "RKLB": {"suppliers": [], "customers": [], "competitors": ["LUNR"], "partners": ["RDW"]},
        "DXYZ": {"suppliers": [], "customers": [], "competitors": [], "related": ["RKLB", "LUNR"]},
    }
...
def main():
    ...
    print(f"  工具: risk-threshold-engine | statsmodels | yfinance | bt | empyrical")
    print("=" * 64)
    print()
    ...
    print(f"  方法: statsmodels Markov Switching")
    ...
    # Markdown table
    print("| 排名 | 标的 | 行业 | 价格 | 综合分 | 建议 | 5D动量 | 20D收益 | 波动率 | Sharpe | 催化 |")
    print("|------|------|------|------|--------|------|--------|---------|--------|--------|------|")
    for i, d in enumerate(decisions):
        action_icon = {"STRONG_BUY": "🟢", "BUY": "🟢", "HOLD": "🟡", "REDUCE": "🟠", "SELL": "🔴"}.get(d["action"], "⚪")
        print(f"| {i+1} | {d['ticker']} | {d['sector']} | ${d['price']:.2f} | {d['composite']:.2f} | {action_icon} {d['action']} | {d['mom_5d']:.1f}% | {d['mom_20d']:.1f}% | {d['ann_vol']:.2f} | {d['sharpe_6m']:.2f} | {d['catalyst_count']} |")
    ...
    if buy_decisions and total_score > 0:
        for d in buy_decisions:
            ...
            cat_desc = ", ".join(c["event"] for c in catalysts[:2]) if catalysts else "无近期催化"
            print(f"  {d['ticker']:6s}: {adjusted_alloc:.0f}% (原始{raw_alloc:.0f}%, 风控-{lot_reduction*100:.0f}%) | {cat_desc}")
            if chain["suppliers"]:
    ...
    outbox_path = outbox_dir / f"DECISION_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    outbox_path.write_text(json.dumps(output, ensure_ascii=False, indent=2), encoding="utf-8")
