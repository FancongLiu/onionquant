...
def check_reddit_sentiment() -> dict:
    ...
    for sr_name in subreddits:
        try:
            ...
            count = 0
            posts = []
            for post in sub.search(" OR ".join(keywords), sort="new", time_filter="day", limit=10):
                count += 1
                posts.append({
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if total_sentiment > 0:
                ...
                result["sentiment"] = "bullish" if ratio > 0.65 else ("bearish" if ratio < 0.35 else "neutral")
...
def check_news_headlines() -> dict:
    ...
    try:
        ...
        news = ticker.news
        spacex_news = [n for n in news if any(
...
def detect_anomalies(current: dict, cache: dict) -> list[dict]:
    ...
    if prev_mentions > 0 and cur_mentions / prev_mentions >= SENTIMENT_THRESHOLDS["reddit_mentions_spike"]:
        alerts.append({
            "level": "🟡 P1",
            "source": "Reddit",
    ...
    if prev_news > 0 and cur_news / prev_news >= SENTIMENT_THRESHOLDS["news_headline_count_spike"]:
        alerts.append({
            "level": "🟡 P1",
            "source": "News",
...
def write_sentiment_alert(alerts: list[dict], current: dict) -> Path | None:
    ...
    for a in alerts:
        lines.append(f"| {a['level']} | {a['source']} | {a['indicator']} | {a['detail']} |")
...
def main():
    ...
    parser.add_argument("--interval", type=int, default=1800, help="Check interval in seconds (default: 1800=30min)")
    ...
    while True:
        try:
            ...
            if alerts:
                ...
                logger.warning("⚠️  %d sentiment anomalies → %s", len(alerts), path.name if path else "none")
            ...
            logger.info(
                ...
                r["mentions"], r["sentiment"], st["message_count"], st["sentiment"], n["count"],
            )
