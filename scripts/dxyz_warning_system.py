...
def write_alert(alerts: list[dict], price: float) -> Path | None:
    ...
    for a in alerts:
        lines.append(
            f"| {a['level']} | {a['indicator']} | {a['value']} | {a['threshold']} | {a['action']} |"
        )
            ...
            if df.empty:
                ...
                if alerts:
                    ...
                    if len(dxyz_df) >= 15:
                        ...
                        logger.info(
                            "✅ DXYZ $%.2f | RSI %.1f | NAV溢价 %.0f%% — all clear",
                            price,
                            rsi,
                            (price / NAV_PER_SHARE - 1) * 100,
                        )
                    else:
                        logger.info(
                            "✅ DXYZ $%.2f — all clear (insufficient data for RSI)",
                            price,
                        )
        except Exception as e:
