...
HISTORICAL_BINARY_EVENTS = {
    "DXYZ": [
        {
            "date": "2025-10-13",
            "event": "Starship IFT-5 发射成功",
            "result": "success",
            "return_3d": 0.22,
        },
        {
            "date": "2025-11-19",
            "event": "Starship IFT-6 发射成功",
            "result": "success",
            "return_3d": 0.15,
        },
        {
            "date": "2026-01-16",
            "event": "Starship IFT-7 发射失败",
            "result": "failure",
            "return_3d": -0.31,
        },
        {
            "date": "2026-03-06",
            "event": "Starship IFT-8 发射失败",
            "result": "failure",
            "return_3d": -0.25,
        },
        {
            "date": "2026-04-17",
            "event": "Starship IFT-9 部分成功",
            "result": "partial",
            "return_3d": -0.05,
        },
        {
            "date": "2026-05-03",
            "event": "Starship IFT-10 成功",
            "result": "success",
            "return_3d": 0.18,
        },
        {
            "date": "2026-05-13",
            "event": "Starship IFT-11 成功 + IPO临近",
            "result": "success",
            "return_3d": 0.42,
        },
    ],
...
def backtest_binary_events(
    close_orig: pd.Series, events: list, entry_days_before: int = 3
) -> dict:
    ...
    return {
        ...
        "sharpe_events": round(float(ret_series.mean() / ret_series.std()), 2)
        if ret_series.std() > 0
        else 0,
        "by_result": {
            "success": {
                "count": len(successes),
                "avg_return": round(float(np.mean([s["return"] for s in successes])), 4)
                if successes
                else 0,
            },
            "failure": {
                "count": len(failures),
                "avg_return": round(float(np.mean([f["return"] for f in failures])), 4)
                if failures
                else 0,
            },
        },
...
def backtest_ipo_analog(analogs: list) -> dict:
    ...
    return {
        ...
        "analogs": [
            f"{a['ticker']} {a['event']}: {a['return_5d']:.0%}" for a in analogs
        ],
...
def monte_carlo_binary_outcome(
    success_pct: float = 0.55,
    failure_pct: float = -0.35,
    success_prob: float = 0.65,
    n_sims: int = 10_000,
    position_size: float = 27840,
) -> dict:
    """蒙特卡洛模拟二元事件结果分布。
    ...
    return {
        ...
        "kelly_fraction": round(
            max(
                0, success_prob - (1 - success_prob) / (success_pct / abs(failure_pct))
            ),
            4,
        ),
    }
...
def main():
    ...
    print("=" * 64)
    print("  工具: bt (pmorissette) | yfinance | empyrical")
    print("=" * 64)
    print()
    ...
    print(
        f"  数据: {len(close)} 个交易日, {close.index[0].strftime('%Y-%m-%d')} → {close.index[-1].strftime('%Y-%m-%d')}"
    )
    ...
    for evt in result["events"]:
        ...
        print(
            f"  {icon} {evt['event']}: {evt['return']:.1%} ({evt['entry_date']} → {evt['exit_date']})"
        )
    ...
    print(
        f"  成功事件 ({result['by_result']['success']['count']}次) 均值: {result['by_result']['success']['avg_return']:.1%}"
    )
    print(
        f"  失败事件 ({result['by_result']['failure']['count']}次) 均值: {result['by_result']['failure']['avg_return']:.1%}"
    )
    print()
    ...
    print(
        f"  成功P&L: ${mc['success_pnl']:+,.0f}  |  失败P&L: ${mc['failure_pnl']:+,.0f}"
    )
    ...
    print("⚖️  风险/回报总结")
    print("-" * 40)
    print("  仓位: $27,840 (585股 × $47.62)")
    print("  成功场景: DXYZ → $60-70 (+25-45%) = +$7,000-12,500")
    print("  失败场景: DXYZ → $30-35 (-26-37%) = -$7,300-10,300")
    print("  硬止损: $38 (-20%) = -$5,568")
