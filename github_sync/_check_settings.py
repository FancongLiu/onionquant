...
ext_path = os.path.expanduser(r'~\.vscode\extensions\github.copilot-chat-0.48.1\package.json')
...
for section in props_list:
    ...
    for k, v in props.items():
        if any(x in k.lower() for x in ['nextedit', 'editsuggest', 'keep', 'accept', 'review', 'confirm']):
