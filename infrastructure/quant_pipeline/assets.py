...
def market_data_ingest(
    context: AssetExecutionContext, config: IngestConfig
) -> pd.DataFrame:
...
def factor_compute(
    context: AssetExecutionContext, config: FactorConfig
) -> pd.DataFrame:
    ...
    context.log.info(
        f"Computing factors on {len(df)} rows across {df['ticker'].nunique()} tickers"
    )
...
def _write_to_timescaledb(df: pd.DataFrame, table: str, context) -> None:
    ...
    try:
        ...
        context.log.warning(
            f"TimescaleDB write failed ({e}); data returned as DataFrame only"
        )
...
def _write_factors_to_timescaledb(factors: pd.DataFrame, context) -> None:
    ...
    try:
        ...
        factor_cols = [
            c for c in factors.columns if c not in id_cols and c != "industry"
        ]
        long = factors.melt(
            id_vars=id_cols,
        ...
        rows = [
            (r["ticker"], r["date"], r["factor_name"], r["value"], None)
...
def _build_report(df: pd.DataFrame) -> str:
    ...
    for ticker in sorted(df["ticker"].unique()):
        ...
        ma20 = (
            tdf["close"].rolling(20).mean().iloc[-1]
            if len(tdf) >= 20
            else tdf["close"].mean()
        )
