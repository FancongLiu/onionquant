            ...
            if "ticker" in df.columns and len(df) > 100:
                ...
                for f in factor_cols[:20]:
                    ...
                    for s in stocks:
                        ...
                        data[f][s] = (
                            float(srows.dropna().iloc[-1])
                            if len(srows.dropna()) > 0
                            else None
                        )
                return {
                    "factors": factor_cols[:20],
                    "stocks": stocks,
                    "data": data,
                    "source": "live",
                }
...
async def quant_signals():
        ...
        try:
            ...
            if "ticker" in df.columns and len(df) > 100:
                ...
                active = filter_factors_by_ic(
                    result, factor_cols, ic_threshold=0.02, min_factors=5
                )
                combined = ic_weighted_combine(
                    result, active, result["close"], ic_threshold=0.0
                )
                signals = generate_signals(
                    combined, "combined_score", top_k=15, method="long_short"
                )
                longs = [
                    {
                    ...
                    for _, row in signals[signals["signal"] == 1]
                    .nlargest(15, "combined_score")
                    .iterrows()
                ]
                shorts = [
                    {
    ...
    return {
        "longs": _make_sigs(15, 1),
        "shorts": _make_sigs(15, -1),
        "source": "generated",
    }
        ...
        try:
            ...
            if "ticker" in df.columns and len(df) > 300:
                ...
                if "close" in df.columns:
                    rolling_ic = rolling_ic_matrix(
                        result, factor_cols, df["close"], window=21
                    )
                    series = {}
                    for col in factor_cols:
                        ...
                        if ic_col in rolling_ic.columns:
                            ...
                            if len(vals) >= 20:
                                ...
                                series[col] = [
                                    round(float(v), 4)
                                    for v in vals.iloc[::step].tolist()
                                ]
                    if series:
...
async def quant_risk():
    try:
        ...
        data_dir = PROJECT_ROOT / "quant_framework" / "data" / "raw"
        price_files = (
            list(data_dir.glob("price_*.parquet")) if data_dir.exists() else []
        )
        if price_files:
            import pandas as pd
            ...
            if "close" in df.columns and len(df) > 50:
                ...
                if len(returns) > 100:
                    ...
                    return {
                        ...
                        "calmar": round(
                            sharpe_ratio(returns.values)
                            / max(abs(max_drawdown(eq)), 0.001),
                            2,
                        ),
                    }
    except Exception:
        ...
        try:
            ...
            if "close" in df.columns and "ticker" in df.columns and len(df) > 100:
                ...
                if not prices.empty and len(prices.columns) >= 3:
                    ...
                    if "weights" in opt:
                        w = opt["weights"]
                        top = sorted(w.items(), key=lambda x: abs(x[1]), reverse=True)[
                            :10
                        ]
                        result["weights"] = [
                        ...
                        result["metrics"] = {
                            "expected_return": round(
                                float(opt.get("expected_return", 0)), 4
                            ),
                            "expected_volatility": round(
                                float(opt.get("expected_volatility", 0)), 4
                            ),
                            "sharpe": round(
                                float(opt.get("expected_return", 0))
                                / max(float(opt.get("expected_volatility", 0)), 0.001),
                                3,
                            ),
                        }
                        result["source"] = "live"
            ...
            if "close" in df.columns and "ticker" in df.columns and len(df) > 100:
                ...
                if not prices.empty and len(prices.columns) >= 3:
                    ...
                    if "weights" in opt:
                        ...
                        top = sorted(
                            zip(tickers_list, w), key=lambda x: abs(x[1]), reverse=True
                        )[:12]
                        result["weights"] = [
                            {"ticker": t, "weight": round(float(wt), 4)}
                            for t, wt in top
                        ...
                        result["metrics"] = {
                            ...
                            "expected_risk": round(
                                float(opt.get("expected_risk", 0)), 4
                            ),
                            "sharpe": round(
                                float(opt.get("expected_return", 0))
                                / max(float(opt.get("expected_risk", 0)), 0.0001),
                                3,
                            )
                            if opt.get("expected_risk", 0) > 0
                            else 0,
                        }
                        result["source"] = "live"
        except Exception as e:
        ...
        try:
            ...
            if "close" in df.columns and "ticker" in df.columns and len(df) > 50:
                ...
                if not prices.empty and len(prices.columns) >= 2:
                    ...
                    result["equity"] = [
                        round(float(v), 4) for v in equity.iloc[::step].tolist()
                    ]
                    result["drawdown"] = [
                        round(float(v), 4) for v in dd.iloc[::step].tolist()
                    ]
                    result["dates"] = [
                        str(d).split(" ")[0] for d in equity.index[::step].tolist()
                    ]
                    result["source"] = "live"
        except Exception as e:
            result["error"] = str(e)[:200]
...
async def strategy_comparison():
        ...
        try:
            ...
            if "close" in df.columns and "ticker" in df.columns and len(df) > 100:
                ...
                if not prices.empty and len(prices.columns) >= 3:
                    ...
                    ew = pd.DataFrame(
                        1.0 / len(tickers), index=prices.index, columns=tickers
                    )
                    strategies["Equal-Weight"] = vectorized_backtest(prices, ew)

                    mom = rets.rolling(21).mean().shift(1)
                    mom_w = mom.div(mom.abs().sum(axis=1), axis=0).fillna(
                        1.0 / len(tickers)
                    )
                    ...
                    rp = (
                        (1.0 / vol.replace(0, np.nan))
                        .div((1.0 / vol.replace(0, np.nan)).sum(axis=1), axis=0)
                    ...
                    for name, s in strategies.items():
                        if "equity_curve" in s and "equity_dates" in s:
                            eq[name] = {
                                "dates": s["equity_dates"],
                                "values": s["equity_curve"],
                            }
                    if eq:
    ...
    if result["source"] == "generated":
        ...
        n = 252
        dates = [
            str(pd.Timestamp("2025-01-01") + pd.Timedelta(days=i)) for i in range(n)
        ]
        eq = {}
        profiles = {
            "Momentum-21d": (0.15, 0.18),
            "Risk-Parity": (0.11, 0.12),
            "Equal-Weight": (0.10, 0.16),
            "Trend-50/200": (0.13, 0.18),
        }
        for name, (ret, vol) in profiles.items():
            ...
            eq[name] = {
                "dates": dates,
                "values": [
                    round(float(v), 2) for v in (100000 * (1 + noise).cumprod())
                ],
            }
        result["equity_curves"] = eq
...
async def param_sweep():
                ...
                if not prices.empty and len(prices.columns) >= 3:
                    ...
                    for lb in lookbacks:
                        ...
                        for th in thresholds:
                            ...
                            for t in tickers:
                                w[t] = np.where(
                                    mom[t] > th, 1.0, np.where(mom[t] < -th, -1.0, 0)
                                )
    ...
    if result["source"] == "generated":
        ...
        result["best"] = {
            "lookback": lookbacks[best_row],
            "threshold": thresholds[best_col],
            "sharpe": heatmap[best_row][best_col],
        }
    ...
    try:
        ...
        if ic_df.empty:
            return {
                "alerts": [],
                "status": "no_data",
                "message": "Insufficient data for IC computation",
            }
    ...
    try:
        ...
        if price_files:
            ...
            if "close" in df.columns and "ticker" in df.columns and len(df) > 200:
                ...
                active = filter_factors_by_ic(
                    factors, factor_cols, ic_threshold=0.02, min_factors=3
                )

                prices = factors.pivot_table(
                    index="date", columns="ticker", values="close"
                ).dropna(axis=1)

                def real_strategy(ic_shrinkage=0.2, ic_horizon=21, top_k=20):
                    result = ic_weighted_combine(
                        factors, active, factors["close"], ic_shrinkage=ic_shrinkage
                    )
                    signals = generate_signals(
                        result, "combined_score", top_k=int(top_k), method="long_only"
                    )
                    pivot_s = (
                        signals.pivot_table(
                            index="date",
...
_FACTOR_HUMAN_NAMES_CN = {
    ...
    "quality_roe": "ROE(质量)",
    "quality_roa": "ROA(质量)",
    "quality_gross_margin": "毛利率(质量)",
    "quality_accruals": "应计利润(质量)",
    "size_log_mcap": "规模因子",
    "liquidity_turnover": "换手率",
    "growth_sue": "标准化意外盈利(增长)",
    "growth_egr": "盈利增长率(增长)",
    "rsi_14": "RSI-14",
    "bb_20_2": "布林带位置",
    "ma_cross_50_200": "均线交叉(50/200)",
    "leverage_de": "负债权益比(杠杆)",
    "beta_63d": "63日Beta",
    "skew_21d": "21日偏度",
    "kurt_21d": "21日峰度",
}
...
def _generate_recommendation_reason(
    ...
    if method == "ic_weighted":
        parts.append(
            f"综合{len(factor_details)}个有效因子计算，{ticker}得分{score:.3f}，整体{direction_word}。"
        )
    elif method == "ml":
...
async def stock_recommendations():
    ...
    if price_files:
        try:
            ...
            active = filter_factors_by_ic(
                factors, factor_cols, ic_threshold=0.02, min_factors=5
            )
            if len(active) < 3:
            ...
            combined = ic_weighted_combine(
                factors, active, factors["close"], ic_threshold=0.0
            )
            signals = generate_signals(
                combined, "combined_score", top_k=15, method="long_short"
            )
            ...
            for _, row in latest.iterrows():
                ...
                for f in active:
                    if f in latest_factors.index and f in factor_means.index:
                        ...
                        z = (
                            (val - float(factor_means[f])) / float(factor_stds[f])
                            if float(factor_stds[f]) > 0
                            else 0
                        )
                ...
                reason = _generate_recommendation_reason(
                    ticker, score, sig, factor_details
                )
                ...
                recommendations.append(
                    {
                        ...
                        "top_factors": sorted(
                            factor_details, key=lambda x: -abs(x["z_score"])
                        )[:5],
                        "reason": reason,
                    }
                )
    ...
    if result["source"] != "live":
        ...
        for i, t in enumerate(tickers_pool[:12]):
            ...
            reason = _generate_recommendation_reason(
                t, score, sig, top_factors, method="ic_weighted"
            )
