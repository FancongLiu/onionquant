...
Usage:
    python company/tools/news_scanner.py --ticker NVDA
    python company/tools/news_scanner.py --watchlist 10
    python company/tools/news_scanner.py --ticker MU --sentiment
"""
...
# 催化剂关键词 (加权)
CATALYST_KEYWORDS = [
    ("earnings", 3.0), ("beat", 2.0), ("miss", -2.0),
    ("guidance", 2.0), ("upgrade", 2.5), ("downgrade", -2.5),
    ("buyback", 1.5), ("dividend", 0.5),
    ("IPO", 2.0), ("merger", 3.0), ("acquisition", 2.5),
    ("FDA", 3.0), ("approval", 2.5),
    ("contract", 2.0), ("partnership", 2.0),
    ("strike", -1.5), ("lawsuit", -2.0), ("investigation", -2.5),
    ("layoff", -1.5), ("bankruptcy", -4.0),
    ("record", 2.0), ("all-time high", 2.0),
    ("selloff", -1.5), ("rout", -2.0), ("plunge", -2.0),
    ("surge", 2.0), ("jump", 1.5), ("rally", 1.5),
    ("AI", 1.0), ("NVIDIA", 1.0), ("GPU", 1.0),
    ("CPO", 1.5), ("optical", 1.0), ("HBM", 1.0), ("packaging", 1.0),
]
...
class NewsScanner:
    ...
    def _vader_sentiment(self, headlines: list[str]) -> dict:
        ...
        return {
            ...
            "positive_pct": round(pos_count / len(headlines) * 100, 1) if headlines else 0,
            "negative_pct": round(neg_count / len(headlines) * 100, 1) if headlines else 0,
            "neutral_pct": round(neu_count / len(headlines) * 100, 1) if headlines else 0,
    ...
    def _keyword_sentiment(self, headlines: list[str]) -> dict:
        ...
        return {
            ...
            "sentiment_label": "Bullish" if avg > 0.5 else "Bearish" if avg < -0.5 else "Neutral",
            "total_headlines": len(headlines),
    ...
    def scan_ticker(self, ticker: str) -> dict:
        ...
        all_headlines = [n["title"] for n in finviz_news] + [n["title"] for n in google_news]
    ...
    def scan_watchlist(self, tickers: list[str] | None = None, max_items: int = 20) -> list[dict]:
        """批量扫描关注列表, 按新闻热度排序."""
        if tickers is None:
    ...
    def get_trending_by_news(self, tickers: list[str] | None = None, top_n: int = 10) -> list[dict]:
        """快速获取新闻最多的 N 只股票."""
        if tickers is None:
            tickers = AI_CHAIN_TICKERS
...
if __name__ == "__main__":
    ...
    p.add_argument("--watchlist", type=int, default=0, metavar="N",
    ...
    if args.ticker:
        ...
        print(f"  数据源: Finviz + Google News RSS (全部免费)")
        print(f"{'='*55}")
        ...
        print(f"  情绪: {result['sentiment']['sentiment_label']} "
        ...
        if args.catalysts and result["catalysts"]:
            ...
            for c in result["catalysts"][:8]:
                ...
                print(f"  {sign}{c['weight']:.1f} [{c['keyword']}] {c['headline'][:100]}")
        ...
        for t in ["NVDA", "MU"]:
            ...
            print(f"${t}: {result['news_count']}条新闻 | "
