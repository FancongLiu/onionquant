...
class FullScanner:
    ...
    def run_heat_dashboard(self):
        ...
        for i, s in enumerate(absolute):
            ...
            print(f"  {i+1}. {s['ticker']:<6} mentions:{s['mentions']:>4} "
                  f"rank:{s['rank']:>4} upvotes:{s['upvotes']:>5} | {subs}")

        # 24h 上升榜
        rising = [s for s in stocks if s.get("rank_change", 0) > 3 and s["mentions"] >= 5]
        ...
        for i, s in enumerate(ai_chain):
            ...
            subs = ", ".join(s.get("subreddits", [])[:3])
            print(f"  {s['ticker']:<6} mentions:{s['mentions']:>4} rank:{s['rank']:>4} "
    ...
    def run_market_heat(self, tickers: list[str] | None = None):
        ...
        if not self.market:
            return
        print(f"\n{'#'*65}")
        print(f"#  LEVEL 2: MARKET HEAT (Real Trading Data)")
        print(f"#  Scale: millions of shares/day → GENUINE BIG DATA")
        print(f"{'#'*65}")
        ...
        for i, r in enumerate(vol_results[:12]):
            ...
            print(f"  {i+1}. {r['ticker']:<6} {icon} {r['volume_ratio']:.1f}x "
    ...
    def run_reddit_scan(self, tickers: list[str] | None = None):
        ...
        if not self.reddit:
            return
        print(f"\n{'#'*65}")
        print(f"#  LEVEL 3: REDDIT REAL-TIME (Public JSON)")
        print(f"{'#'*65}")
        ...
        for ticker in tickers:
            ...
            if data["total_posts"] >= 2:
                results.append(data)
                print(f"  {ticker:<6} {data['total_posts']:>2}帖 "
    ...
    def run_news_scan(self, tickers: list[str] | None = None):
        ...
        if not self.news:
            return
        print(f"\n{'#'*65}")
        print(f"#  LEVEL 4: NEWS SENTIMENT (Finviz)")
        print(f"{'#'*65}")
        ...
        for ticker in tickers:
            ...
            if r["news_count"] > 0:
                results.append(r)
                print(f"  {ticker:<6} {r['news_count']:>3}条 "
    ...
    def run_cross_validation(self, top_n: int = 16):
        ...
        print(f"\n{'#'*65}")
        print(f"#  LEVEL 5: CROSS-VALIDATION (4 sources)")
        print(f"{'#'*65}")
        ...
        for s in ai_chain:
            ...
            news_sent = news_data.get(ticker, {}).get("sentiment", {}).get("avg_compound", 0)
            ...
            print(f"  {ticker:<6} Δ{ape_str:>5} {vol_str:>7} "
    ...
    def run_full(self, save: bool = False, skip_slow: bool = False):
        ...
        start = time.time()
        print(f"\n{'='*65}")
        print(f"  OnionQuant FULL SCAN ENGINE v3.0")
        print(f"  Time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
        print(f"  Sources: ApeWisdom(v2) + MarketHeat + Reddit + Finviz News")
        print(f"  Cost: FREE (no API keys, no registration)")
        print(f"  Data Scale: social(Reddit) + trading(millions of shares) + news")
        print(f"{'='*65}")
        ...
        print(f"  Data sources: 4 | Tickers tracked: 600+ | AI chain: {len(AI_CHAIN_TICKERS)}")
        print(f"{'='*65}")

        if save:
            ts = datetime.now().strftime("%Y%m%d_%H%M")
            path = DATA_DIR / f"full_scan_{ts}.json"
            path.write_text(json.dumps(self.results, indent=2, ensure_ascii=False, default=str), "utf-8")
            print(f"\n  [SAVE] {path}")
    ...
    def deep_dive(self, ticker: str):
        ...
        if self.market:
            ...
            if opt["unusual_trades"] > 0:
                print(f"       Options: {opt['unusual_trades']} unusual trades | "
                      f"C:${opt['call_premium']:,.0f} P:${opt['put_premium']:,.0f} "
                      f"| {opt['flow_bias']}")
                for t in opt["top_trades"][:3]:
                    print(f"       {t['side']} ${t['strike']} {t['expiration']} "
        ...
        if self.reddit:
            ...
            if data["top_posts"]:
                for p in data["top_posts"][:5]:
                    ...
                    print(f"  [{p['score']}↑] r/{p['subreddit']}{tag} — {p['title'][:80]}")
        ...
        if self.news:
            ...
            if r["catalysts"]:
                ...
                for c in r["catalysts"][:5]:
                    ...
                    print(f"  {sign}{c['weight']:.1f} [{c['keyword']}] {c['headline'][:80]}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--quick", action="store_true", help="快速模式 (热度+市场, 跳过Reddit/News)")
