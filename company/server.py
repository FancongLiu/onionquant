...
# Extracted to company/harness/inbox_processor.py (P0-2 refactor)
# This keeps server.py under 2000 lines
...
# Initialize inbox processor with project paths
inbox_setup(
    project_root=PROJECT_ROOT,
    task_queue_file=TASK_QUEUE_FILE,
    outbox_dir=OUTBOX_DIR,
    deepseek_api_key=DEEPSEEK_API_KEY,
    ctx_state_path=CTX_STATE_PATH,
    langgraph_reports_dir=LANGGRAPH_REPORTS_DIR,
)


async def _process_inbox_message(filepath: Path, text: str, urgent_flag: bool = False):
    """Background task: delegate to inbox_processor module."""
    from company.harness.inbox_processor import process_message
    await process_message(filepath, text, urgent_flag, notify_fn=notify_all)
