...
# ---- 1. CPPI ----
def cppi(equity_curve, floor_pct=0.80, multiplier=3.0,
         risky_return=None, safe_return=0.02 / 252):
    ...
    if risky_return is not None:
        ...
        return {"equity": eq_s, "risky_weight": rw, "floor": fl,
                "cushion": cush, "exposure": exposure}
    cush = np.zeros(T)
    rw = np.zeros(T)
    for t in range(T):
        c = max(eq[t] - floor, 0)
    ...
    return {"equity": eq, "risky_weight": rw,
            "floor": np.full(T, floor), "cushion": cush,
            "exposure": multiplier * cush}


# ---- 2. 移动止损 ----
def moving_stop_loss(equity_curve, lookback=20, multiplier=2.0,
                     ohlc=None):
    """
    移动止损: 支持两种模式
    ...
    if ohlc is not None:
        ...
        for t in range(1, T):
            ...
            if sig[t - 1] == 0:
                ...
                sig[t] = 0
        return {"stop_level": sl, "signal": sig, "trailing_peak": pk,
                "drawdown_from_peak": (eq - pk) / pk, "atr": atr}
    else:
...
# ---- 3. 波动率目标 ----
def volatility_targeting(returns, target_vol=0.15, lookback=21,
                         max_leverage=1.0, ppy=252):
    ...
    sr = pos * r
    return {"position": pos, "realized_vol": rv,
            "scaled_returns": sr, "target_vol": target_vol}
