...
def vectorized_backtest(prices: pd.DataFrame, signals: pd.DataFrame,
    ...
    if len(common_dates) < 20 or len(common_tickers) == 0:
        return {"error": f"insufficient overlap: {len(common_dates)} dates, {len(common_tickers)} tickers"}
...
def signal_backtest(returns: pd.Series, weights: pd.Series,
                    capital: float = 100_000, commission: float = 0.001,
                    ppy: int = 252) -> Dict:
    """单资产信号回测: 信号方向 × 收益率。
...
def event_driven_backtest(ohlcv: pd.DataFrame, strategy_class,
                          capital: float = 100_000, commission: float = 0.001,
    ...
    return {
        ...
        "win_rate": round(
            trades.get("won", {}).get("total", 0) / max(trades.get("total", {}).get("total", 1), 1), 4
        ),
    }


# ─── Metrics (empyrical) ─────────────────────────────────────────
def _compute_metrics(returns: pd.Series, equity: pd.Series,
                     ppy: int, capital: float) -> Dict:
    ...
    profit_factor = abs(avg_win * win_mask.sum() / (avg_loss * (~win_mask).sum())) \
        if (~win_mask).any() and avg_loss != 0 else np.inf
...
def _make_demo_data(n: int = 500, seed: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    prices = pd.DataFrame({
        "ASSET_A": 100 * (1 + np.random.normal(0.08 / 252, 0.20 / np.sqrt(252), n)).cumprod(),
        "ASSET_B": 100 * (1 + np.random.normal(0.06 / 252, 0.25 / np.sqrt(252), n)).cumprod(),
        "ASSET_C": 100 * (1 + np.random.normal(0.10 / 252, 0.30 / np.sqrt(252), n)).cumprod(),
    }, index=dates)
...
def main():
    ...
    parser.add_argument("--signals", help="Parquet/CSV of position weights (dates × tickers)")
    ...
    if args.signals and args.prices:
        sig = pd.read_parquet(args.signals) if args.signals.endswith(".parquet") \
            else pd.read_csv(args.signals, index_col=0, parse_dates=True)
        prc = pd.read_parquet(args.prices) if args.prices.endswith(".parquet") \
            else pd.read_csv(args.prices, index_col=0, parse_dates=True)
    else:
        print("No --signals/--prices; using demo data\n")
        prc, sig = _make_demo_data()

    result = vectorized_backtest(prc, sig, capital=args.capital,
    ...
    for k, v in result.items():
        if isinstance(v, float):
            print(f"  {k:22s}: {v:>12.4f}" if abs(v) < 100 else
                  f"  {k:22s}: {v:>12,.2f}")
        else:
            print(f"  {k:22s}: {v}")
