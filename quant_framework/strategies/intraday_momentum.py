...
def fetch_data(ticker=TICKER, period=PERIOD, interval=INTERVAL):
    ...
    intra = yf.download(ticker, period=period, interval=interval,
...
def calc_indicators(df, daily):
    ...
    df['sigma_open'] = (
        df.groupby(time_key)['move_open']
        .transform(lambda x: x.rolling(ROLL_WIN, min_periods=ROLL_WIN - 1)
                   .mean().shift(1))
    )
    return df
...
def run_backtest(df):
    ...
    class NoiseAreaStrategy(bt.Strategy):
        ...
        def next(self):
            ...
            vol_mult = min(self.p.target_vol / spx_vol, self.p.max_leverage) if not np.isnan(spx_vol) else 1.0
    ...
    daily_bars = df.groupby('date').agg(
        open=('open', 'first'),
        high=('high', 'max'),
        low=('low', 'min'),
        close=('close', 'last'),
