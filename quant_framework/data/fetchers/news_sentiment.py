...
def fetch_news_sentiment(tickers: List[str], days: int = 7, api_key: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        resp = requests.get("https://www.alphavantage.co/query", {
            "function": "NEWS_SENTIMENT", "tickers": ",".join(tickers),
            "apikey": key, "limit": max(50, days * 10), "sort": "LATEST",
        }, timeout=20)
...
_FALLBACK_HEADLINES = {
    "SPY": ["S&P 500 rallies on strong earnings reports", "Market volatility spikes amid Fed uncertainty",
            "Index funds continue to attract record inflows", "Broad market sell-off on recession fears",
            "S&P 500 reaches new all-time high", "Investors rotate into defensive sectors"],
    "QQQ": ["Tech stocks surge on AI boom optimism", "Nasdaq falls as rate hike concerns weigh",
            "Cloud computing sector shows robust growth", "Semiconductor stocks lead market rally",
            "Big Tech earnings beat Wall Street expectations", "Tech sell-off on valuation concerns"],
    "AAPL": ["Apple unveils new product line to strong reception", "iPhone sales exceed analyst forecasts",
             "Apple services revenue hits record high", "Supply chain concerns weigh on Apple stock"],
    "TSLA": ["Tesla deliveries beat quarterly estimates", "EV competition intensifies in key markets",
             "Tesla announces new battery technology breakthrough", "Regulatory scrutiny on self-driving features"],
    "NVDA": ["NVIDIA reports explosive data center revenue growth", "AI chip demand continues to outstrip supply",
             "NVIDIA unveils next-generation GPU architecture", "Analysts raise price targets on AI momentum"],
    "GME": ["GameStop reports surprise quarterly profit", "Retail traders return to meme stocks",
            "GameStop transforms business model toward e-commerce", "Meme stock volatility attracts regulatory attention"],
}
...
def _demo_data(tickers: List[str], days: int) -> pd.DataFrame:
    ...
    for sym in tickers[:6]:
        ...
        for d in range(min(days, len(headlines))):
            ...
            records.append({"ticker": sym, "title": title,
                            "summary": f"[FinBERT fallback] pos={scores['positive']:.3f} neg={scores['negative']:.3f}",
                            "source": "FinBERT-fallback", "published_at": base - timedelta(days=d),
                            "av_sentiment_score": round(score, 4), "av_sentiment_label": label,
                            "relevance_score": 0.5})
...
def aggregate_by_ticker(df: pd.DataFrame, window_days: int = 5) -> pd.DataFrame:
    ...
    grouped = (df.groupby(["ticker", "date"])
               .apply(lambda g: pd.Series(aggregate_sentiments(
    ...
    for col in ["weighted_score", "positive_ratio", "negative_ratio"]:
        grouped[f"rolling_{col}"] = (grouped.groupby("ticker")[col]
