...
**权限模型**: Codex 拥有完全操作权限，但必须自我判断安全等级。权限是用户给的，判断力必须是自己的。
...
- **AGENTS.md 引用此策略的 agent: 每次派发子 Agent 前必须先自问 "串行能不能满足？能就不并行"**
...
**认领步骤**（AGENTS.md 最高优先级规则 — 先于任何其他任务执行）：
...
**数据来源**: Codex 官方博客 + LMCache Blog 实测 trace 数据 + DeepSeek V4 Prompt Cache 诊断 (GitHub)。
...
- Agent 子任务 fork 自主会话 → 继承父会话 prompt prefix（system prompt + tools + AGENTS.md + memory）→ 全部缓存命中
...
**禁止事项**：
- ❌ 禁止启动 WSL screen/tmux 里的独立 `Codex` 会话
- ❌ 禁止用 `Codex -p` 在 bash 循环里轮询
