...
Additional targeted JCI check: 6 packaged-rooftop samples were downloaded and PDF-text verified with `High confidence` without JCI/JACI login. The reusable method is now in `LOCAL_AI_PROMPT_SANITIZED.md`: use Johnson Controls public Fluid Topics API (`clustered-search` -> `documents/{documentId}` -> `documents/{documentId}/content`) and `files.hvacnavigator.com/p/*.pdf` direct PDFs before declaring a login wall.