...
def process_jci(entry):
    ...
    if not results:
        for model in models[:2]:
            results.append(
                f"https://cgproducts.johnsoncontrols.com/MET_PDF/{model}.pdf"
            )
...
def main():
    ...
    pending = sorted(
        ...
        key=lambda x: (
            brand_order.index(x[1]["brand"]) if x[1]["brand"] in brand_order else 99,
            x[1]["series"],
        ),
    )
    ...
    for i, (mid, entry) in enumerate(pending):
        ...
        log.info(
            f"[{i + 1}/{len(pending)}] {brand}/{series} ({len(entry.get('models', []))} models)"
        )
        ...
        try:
            ...
            if not success:
                ...
                entry["evidence"] = (
                    f"No PDF found from structured sources ({len(candidates)} candidates tried)"
                )
    ...
    for b in brand_order:
        ...
        fs = sum(
            ...
            if v.get("brand") == b and v.get("status") == "found"
        )
        if bs > 0:
            log.info(f"  {b}: {fs}/{bs}")
