...
def process_carrier():
    ...
    log.info(
        f"Processing Carrier: {sum(len(v) for v in families.values())} entries in {len(families)} families"
    )
    ...
    for fam in families:
        ...
        pdfs = re.findall(
            r'href="(https?://www\.shareddocs\.com/hvac/docs/[^"]+\.pdf)"', html
        )
        pd_pdfs = [
            p
            for p in pdfs
            if "PD" in p.split("/")[-1].upper() and "PD.pdf" in p.lower() or "PD." in p
        ]
        if not pd_pdfs:
            # Broader search
        ...
        log.info(
            f"  Family {fam}: {len(pdfs)} PDFs, {len(pd_pdfs)} Product Data from {url}"
        )
        for p in pd_pdfs:
    ...
    for fam, entries in families.items():
        ...
        for mid, entry in entries:
            ...
            if caps:
                cap_str = str(caps[0])
                cap_nums = [
                    int(n) for n in re.findall(r"(\d+)", cap_str) if int(n) > 10
                ]
                if cap_nums:
